...
impl ReissueTransaction {
    ...
    fn validate_balance(&self) -> Result<()> {
        let inputs = Amount::checked_sum(self.inputs.iter().map(Dbc::amount))?;
        let outputs = Amount::checked_sum(self.outputs.iter().map(DbcEnvelope::amount))?;
...
mod tests {
    ...
    fn reissue_genesis_multi_output() -> Result<(), Error> {
        ...
        let outputs_sum = Amount::checked_sum(dbcs.iter().map(Dbc::amount))?;
    ...
    fn prop_splitting_the_genesis_dbc(output_amounts: TinyVec<TinyInt>) -> Result<(), Error> {
        ...
        assert_eq!(
            Amount::checked_sum(output_dbcs.iter().map(Dbc::amount))?,
