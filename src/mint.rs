...
use crate::{
    ...
    PublicKey, PublicKeySet, Result, SpendBook, SpendKey,
};
...
impl<K: KeyManager, S: SpendBook> MintNode<K, S> {
    ...
    pub fn reissue(
        ...
        for input_dbc in reissue_req.transaction.inputs.iter() {
            let spend_key = input_dbc.spend_key();
            match reissue_req.input_ownership_proofs.get(&spend_key) {
                Some(sig) if spend_key.0.verify(sig, &input_dbc.spend_key().0.to_bytes()) => {}
                Some(_) => return Err(Error::FailedMintSignature),
...
mod tests {
    ...
    use crate::{
        ...
        Amount, AmountCounter, DbcBuilder, Output, ReissueRequestBuilder, SimpleKeyManager, SimpleSigner,
        SimpleSpendBook, TransactionBuilder,
    };
    ...
    fn reissue_genesis() -> Result<(), Error> {
        ...
        let rs = genesis_node.reissue(rr, BTreeSet::from_iter([genesis_dbc.spend_key()]))?;
    ...
    fn reissue_genesis_and_child() -> Result<(), Error> {
        ...
        let rs = genesis_node.reissue(rr, BTreeSet::from_iter([dbc_a.spend_key()]))?;
    ...
    fn prop_splitting_the_genesis_dbc(output_amounts: TinyVec<TinyInt>) -> Result<(), Error> {
        ...
        let reissue_share =
            match genesis_node.reissue(reissue_req, BTreeSet::from_iter([genesis_spend_key])) {
                Ok(rs) => {
