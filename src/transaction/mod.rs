...
impl RevealedTransaction {
    pub fn sign(
        ...
        let blinded_outputs = self.blinded_outputs(&mut rng)?;
        ...
        let revealed_output_amounts = self
            .outputs
            .iter()
    ...
    fn blinded_outputs(&self, mut rng: impl RngCore + CryptoRng) -> Result<Vec<BlindedOutput>> {
        let mut prover_ts = Transcript::new(MERLIN_TRANSCRIPT_LABEL);
