...
impl RevealedTransaction {
    pub fn sign(
        ...
        let revealed_outputs = self.revealed_outputs(&revealed_input_amounts);
        let output_proofs = self.output_range_proofs(&revealed_outputs, &mut rng)?;
        ...
        let revealed_output_amounts = revealed_outputs
            .iter()
    ...
    fn revealed_outputs(&self, revealed_input_amounts: &[RevealedAmount]) -> Vec<RevealedOutput> {
        // Avoid subtraction underflow in next step.
        if self.outputs.is_empty() {
            return vec![];
        }

        let mut revealed_outputs: Vec<RevealedOutput> = self
            .outputs
            .iter()
