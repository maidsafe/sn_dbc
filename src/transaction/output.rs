...
impl DbcTransaction {
    ...
    pub fn gen_message(&self) -> Vec<u8> {
        // All public keys
        let public_keys: Vec<G1Affine> = self.inputs.iter().map(|m| m.public_key).collect();
    ...
    pub fn verify(&self, public_commitments: &[G1Affine]) -> Result<()> {
        // check input sigs
        ...
        let pk_unique: BTreeSet<_> = self
            .inputs
            .iter()
            .map(|input| input.public_key.to_compressed())
            .collect();
