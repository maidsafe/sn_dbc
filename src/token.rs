    ...
    fn from_str(value_str: &str) -> Result<Self> {
        ...
        let converted_units = {
            let units = itr
                ...
                .ok_or_else(|| Error::FailedToParseToken("Can't parse token units".to_string()))?;

            units
                .checked_mul(TOKEN_TO_RAW_CONVERSION)
                .ok_or(Error::ExcessiveTokenValue)?
        };
...
mod tests {
    ...
    fn from_str() -> Result<()> {
        ...
        assert_eq!(
            Err(Error::LossOfTokenPrecision),
            Token::from_str("0.0000000009")
        );
        assert_eq!(
            Err(Error::ExcessiveTokenValue),
            Token::from_str("18446744074")
        );
        Ok(())
    }
