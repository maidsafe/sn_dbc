...
impl CashNote {
    ...
    pub fn id(&self) -> UniquePubkey {
        self.id
    }

    // Return MainPubkey from which UniquePubkey is derived.
    pub fn public_address(&self) -> &MainPubkey {
        &self.public_address
    }
    ...
    /// Return the Tokens for this CashNote.
    pub fn token(&self) -> Result<Nano> {
        Ok(self
            .src_tx
            .outputs
            .iter()
            .find(|o| &self.id() == o.cashnote_id())
    ...
    pub fn hash(&self) -> Hash {
        ...
        sha3.update(&self.public_address.to_bytes());
    ...
    pub fn verify(&self, main_key: &MainSecretKey) -> Result<(), Error> {
        ...
        let cashnote_id = self.derived_key(main_key)?.unique_pubkey();
        if !self
            .src_tx
            .outputs
            .iter()
            .any(|o| cashnote_id.eq(o.cashnote_id()))
        {
        ...
        if !self.signed_spends.iter().all(reasons_are_equal) {
            return Err(Error::SignedSpendReasonMismatch(cashnote_id));
        }
        Ok(())
    }
...
pub(crate) mod tests {
    ...
    #[test]
    fn input_should_error_if_cashnote_id_is_not_derived_from_main_key() -> Result<(), Error> {
        let mut rng = crate::rng::from_seed([0u8; 32]);
    ...
    pub(crate) fn generate_cashnote_of_value_from_pk_hex(
        ...
        let public_address = MainPubkey::new(pk);
        generate_cashnote_of_value(amount, public_address, rng)
    }
